...
namespace test {
    automata::codegen::CompiledAutomaton createLLVMCompiledAutomaton(const std::unique_ptr<automata::parsing::LikePatternAutomaton>& automaton, bool enableSIMD) {
        automata::codegen::llvmir::LLVMCompiler compiler{enableSIMD, false};
        std::unique_ptr<automata::codegen::Parser> parser = compiler.compile(automaton);
        return automata::codegen::CompiledAutomaton{parser};
    }

    TEST(LLVMCompiledPatternMatchTest, TestStartLongnamePatternsNoSIMD) {
        ...
        for (size_t _ = 0; _ < numPatterns; ++_) {
            ...
            automata::codegen::CompiledAutomaton automaton = createLLVMCompiledAutomaton(automata::parsing::LikePatternAutomaton::build(bytes, currentEncoder), false);
    ...
    TEST(LLVMCompiledPatternMatchTest, TestStartLongnamePatternsSIMDEnabled) {
        std::string path = "../../data/test_data/StartFullPatternMatchingTest";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
        ...
        for (size_t _ = 0; _ < numPatterns; ++_) {
            ...
            automata::codegen::CompiledAutomaton automaton = createLLVMCompiledAutomaton(automata::parsing::LikePatternAutomaton::build(bytes, currentEncoder), true);
    ...
    TEST(LLVMCompiledPatternMatchTest, TestEndLongnamePatternsNoSIMD) {
    ...
    TEST(LLVMCompiledPatternMatchTest, TestEndLongnamePatternsSIMDEnabled) {
        std::string path = "../../data/test_data/EndFullPatternMatchingTest";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
    ...
    TEST(LLVMCompiledPatternMatchTest, TestFirstnamePatternsNoSIMD) {
    ...
    TEST(LLVMCompiledPatternMatchTest, TestFirstnamePatternsSIMDEnabled) {
        std::string path = "../../data/test_data/FullPatternMatchTestTestFirstnamePatterns";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
    ...
    TEST(LLVMCompiledPatternMatchTest, TestHamletPatternsNoSIMD) {
    ...
    TEST(LLVMCompiledPatternMatchTest, TestHamletPatternsSIMDEnabled) {
        std::string path = "../../data/test_data/FullPatternMatchTestTestHamletPatterns";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
    ...
    TEST(LLVMCompiledPatternMatchTest, TestL_commentPatternsNoSIMD) {
    ...
    TEST(LLVMCompiledPatternMatchTest, TestL_commentPatternsSIMDEnabled) {
        std::string path = "../../data/test_data/FullPatternMatchTestTestL_commentPatterns";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
    ...
    TEST(LLVMCompiledPatternMatchTest, TestPs_commentPatternsNoSIMD) {
    ...
    TEST(LLVMCompiledPatternMatchTest, TestPs_commentPatternsSIMDEnabled) {
        std::string path = "../../data/test_data/FullPatternMatchTestTestPs_commentPatterns";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
    ...
    TEST(LLVMCompiledPatternMatchTest, TestChinesePatternsNoSIMD) {
    ...
    TEST(LLVMCompiledPatternMatchTest, TestChinesePatternsSIMDEnabled) {
        std::string path = "../../data/test_data/FullPatternMatchTestTestChinesePatterns";
        int fd = open(path.c_str(), O_RDONLY);
        if (fd == -1) {
            throw std::runtime_error(fmt::format("Could not open {}", path));
